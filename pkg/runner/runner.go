...
// ListenAndServe - run http server
func ListenAndServe(ctx context.Context, addr string, handler http.Handler) error {
	srv := &http.Server{
		Addr:         addr,
		Handler:      handler,
		WriteTimeout: time.Duration(config.GetInt("http_rw_timeout")) * time.Second,
		ReadTimeout:  time.Duration(config.GetInt("http_rw_timeout")) * time.Second,
		IdleTimeout:  time.Duration(config.GetInt("http_idle_timeout")) * time.Second,
	}
	go func() {
